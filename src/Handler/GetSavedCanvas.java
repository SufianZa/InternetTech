 ...
 */
public class GetSavedCanvas implements IHandler {
    String[] arraySplited;
    String html ="";
    ...
    public GetSavedCanvas(String[] arraySplited) {
    ...
    public void handle(OutputStream out) throws IOException {
        switch (arraySplited[0]) {
            case "GET":
               int a = Integer.parseInt(arraySplited[1].substring(arraySplited[1].indexOf("id=") + 3));
                if(GenerateID.searchID(a)) html +=      "var id ="+a+"\n";
                else {
                   new BadRequestHandler().handle(out);
                    return;

                }
               for(JSONSerialize obj : CanvasSave.coordinates) {
                   if (obj.getKey("id").equals(a)) {
                       html +=
                                        "  context.beginPath();\n" +
